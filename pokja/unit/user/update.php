...
if (isset($_GET['id_user'])) {
    ...
    if (!$data) {
        header('Location: main_pokja.php?unit=user&err=Data user tidak ditemukan!');
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($password)) {
        ...
        $query = "UPDATE tb_user 
                  SET nama_lengkap='$nama_lengkap', username='$username', password='$hashed_password' WHERE id_user='$id_user'";
    } else {
    ...
    if (mysqli_query($config, $query)) {
        header('Location: main_pokja.php?unit=user&msg=Data user berhasil diperbarui!');
        exit;
    } else {
        $errMsg = urlencode('Gagal menyimpan data: ' . mysqli_error($config));
			header('Location: main_pokja.php?unit=user&err=' . $errMsg);
			exit;
    }
}
?>
        ...
        <div class="card card-default">
            ...
            <form method="post">
                    ...
                    <div class='row'>
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label>Kode Pokja <small>(Tidak Bisa diedit)</small></label>
                                <input type="text" name="kode_poka" class="form-control" 
                                       value="<?php echo htmlspecialchars($data['kode_pokja']); ?>" readonly>
                            </div>
                            <div class="form-group">
                ...
                <div class="card-footer">
                    <a class="btn btn-app bg-warning float-left" href="main_pokja.php?unit=user">
