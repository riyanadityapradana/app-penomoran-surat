<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (mysqli_num_rows($cek) > 0) {
			header('Location: main_admin.php?unit=pokja&err=Username sudah digunakan, silakan pilih yang lain!');
			exit;
		}
		...
		if (mysqli_query($config, $query)) {
			header('Location: main_admin.php?unit=pokja&msg=User baru untuk pokja berhasil ditambahkan!');
			exit;
		} else {
			$errMsg = urlencode('Gagal menyimpan data: ' . mysqli_error($config));
			header('Location: main_admin.php?unit=create_pokja&err=' . $errMsg);
			exit;
		}
	}
?>
		...
		<div class="container-fluid">
			<div class="card card-default">
				<div class="card-header">
					<h3 class="card-title">Silahkan Input Data Program Kerja</h3>
				</div>
						...
						<div class='row'>
							<div class="col-sm-12">
								<div class="form-group">
									...
									<input type="text" name="kode_pokja" placeholder="Contoh: PMKP, TKRS, dll" class="form-control" required>
								</div>
								<div class="form-group">
